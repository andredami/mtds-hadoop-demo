...
parsed_datarecord = FOREACH datarecords GENERATE (timestamp-start.timestamp)/3600 AS hourindex:int, plugid, householdid, houseid, measurement;
data_per_househour= GROUP parsed_datarecord BY (houseid, hourindex);
house_median = FOREACH data_per_househour GENERATE group, AVG(parsed_datarecord.measurement) AS house_median:float;
data_per_plughour=  GROUP parsed_datarecord BY (houseid, hourindex, householdid, plugid);
plug_median = FOREACH data_per_plughour GENERATE group, AVG(parsed_datarecord.measurement) AS plug_median:float;
...
outliers = FOREACH pre_outliers GENERATE data_per_househour.hourindex, data_per_househour.houseid, (data_per_plug_hour.plug_median - data_per_house_hour.house_median) AS test:int;
